...
public class Test2 {
    ...
    @Test
    public void test() {
    	String query = "SELECT \\ALL * Dimension4\\";
    	Parser p = new Parser();
        ...
        for(int i=0; i<aexpqueries.size()-1;i++){
            try {
            	//VIRTUAL DIM NOT IMPLEMENTED
            	exception.expect(NullPointerException.class);
                Assert.assertEquals("{resultTables:null resultFields:[Field1, Field3]}", (new AExpProcessor()).process(aexpqueries.get(i),true).toString());
