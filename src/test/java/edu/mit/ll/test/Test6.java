    ...
    public void test() throws JsonSyntaxException, CannotProceedException {
        ...
        for(int i=0; i<aexpqueries.size()-1;i++){
            try {
            	...
            	//The output here does not include durations thats why it doesnt match (fails test)
                Assert.assertEquals("{resultTables:[Table [durations=null, fields=[Field1, Field2], name=EventTable1]] resultFields:null}", r1);
